...
public class OrderServiceImplTest {
    ...
    public void testSendTicket() throws Exception {
        ...
        Optional<Place> place1 = Optional.of(place);
        ...
        for (int i = 0; i < orderItem.getQuantity(); i++) {
            ...
            ticket.setCode(codes.get(i).getCode());
            ticket.setRedemptionDate(order1.getRedemptionDate());
            ticket.setOrderItem(orderItem);
            ticket.setVisitorTypeId(visitorType.getId());
            tickets.add(ticket);
        }
        when(mockTicketRepository.saveAll(tickets)).thenReturn(tickets);

        File file = new File("Test.pdf");
        List<PrintRequest> printRequests = new ArrayList<>();
        PrintRequest printRequest = new PrintRequest();
        printRequest.setRedemptionDate(order1.getRedemptionDate());
        ...
        orderDTO.setOrderCode("orderCode");
        orderDTO.setTotalPayment(0);
        when(mockOrderConverter.toDTO(order1)).thenReturn(orderDTO);

        // Run the test
        final ResponseEntity<?> result = orderServiceImplUnderTest.sendTicket(0L);

        // Verify the results
        verify(mockCodeRepository).deleteAll(codes);
