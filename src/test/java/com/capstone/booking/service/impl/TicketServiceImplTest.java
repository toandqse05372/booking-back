...
public class TicketServiceImplTest {
    ...
    public void testUpdate() {
        ...
        orderItem1.setOrder(order1);
        orderItem1.setTicket(new HashSet<>(Arrays.asList(new Ticket())));
        ticket2.setOrderItem(orderItem1);
        when(mockTicketConverter.toTicket(eq(new TicketDTO()), any(Ticket.class))).thenReturn(ticket2);
        ...
        ticket3.setOrderItem(orderItem2);
        when(mockTicketRepository.save(any(Ticket.class))).thenReturn(ticket3);

        // Configure TicketConverter.toDTO(...).
        final TicketDTO ticketDTO1 = new TicketDTO();
        ticketDTO1.setRedemptionDate(new GregorianCalendar(2019, Calendar.JANUARY, 1).getTime());
        ticketDTO1.setCode("code");
        final VisitorTypeDTO visitorType4 = new VisitorTypeDTO();
        visitorType4.setTypeName("typeName");
        visitorType4.setTypeKey("typeKey");
        visitorType4.setTicketTypeId(0L);
        visitorType4.setPrice(0);
        visitorType4.setBasicType(false);
        visitorType4.setRemaining(0);
        visitorType4.setStatus("status");
        ticketDTO1.setVisitorType(visitorType4);
        ticketDTO1.setVisitorTypeId(0L);
        when(mockTicketConverter.toDTO(any(Ticket.class))).thenReturn(ticketDTO1);

        // Run the test
        final ResponseEntity<?> result = ticketServiceImplUnderTest.update(ticketDTO);
    ...
    @Test
    public void testCreateReport_JavaMailSenderThrowsMailException() throws Exception {
        // Setup
        final OutputReport report = new OutputReport();
        final ReportItem item = new ReportItem();
        item.setTicketTypeName("ticketTypeName");
